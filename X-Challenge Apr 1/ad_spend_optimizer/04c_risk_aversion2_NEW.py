...
# MAGIC        sum(transition_probability)
# MAGIC FROM
...
transitions_detail10 = spark.sql(
"""
select A.uid, A.start_state, A.total_transitions_per_user,
   B.avg_transitions_per_chanel, B.std_transitions_per_channel
FROM 
   transition_matrix_detail2 A
LEFT JOIN
(select distinct start_state,
      avg(total_transitions_per_user) avg_transitions_per_chanel,
      std(total_transitions_per_user) std_transitions_per_channel
FROM
  transition_matrix_detail2
group by start_state
) B
ON A.start_state = B.start_state
order by start_state
"""
).drop_duplicates(subset = ["uid", "start_state"])

transitions_detail10 = (transitions_detail10
.withColumn("thresh_per_channel", col('avg_transitions_per_chanel') - col('std_transitions_per_channel'))
.withColumn("thresh_per_channel", when(col("thresh_per_channel")>0, col("thresh_per_channel")).otherwise(0))
                        .filter("start_state != 'Start'")
.withColumn("risk_score", col("total_transitions_per_user") / col("thresh_per_channel"))
)
transitions_detail10.createOrReplaceTempView("trans_detail10")
display(transitions_detail10.sort("uid", "start_state"))

# COMMAND ----------

risk_scores = spark.sql(
"""
SELECT uid, max(total_transitions_per_user) max_transitions, 
...
user_rankings = spark.sql(
"""
select uid, VISIT_RANK2
from user_visit_rankings
"""
)
user_probs = spark.sql("""select * from user_prob_table""")

risk_scores2 = risk_scores.join(user_rankings, on = ['uid'], how = 'left')
risk_scores3 = risk_scores2.join(user_probs, on = ['uid'], how = 'left').fillna(1, subset = ['prob_to_browse'])
risk_scores3.createOrReplaceTempView("risk_scores3")
display(risk_scores3)

# COMMAND ----------


user_risk_aversion = spark.sql("""
select VISIT_RANK2, sum(max_transitions) as transitions,
            avg(risk_score) as risk_score, max(risk_score) as max_risk_score ,
            std(prob_to_browse) as std_prob, 100 * std(prob_to_browse) * std(prob_to_browse) as std_prob2
from risk_scores3
group by VISIT_RANK2
"""
)
display(user_risk_aversion)

# COMMAND ----------

risk_table_name = "user_risk_aversion"

(user_risk_aversion
   .write
   .format("delta")
   .saveAsTable(risk_table_name)
)

# COMMAND ----------

# MAGIC %sql
# MAGIC select * from user_visit_rankings
# MAGIC         

# COMMAND ----------

# MAGIC %sql
# MAGIC select * from user_prob_table

# COMMAND ----------

# MAGIC %sql
# MAGIC select uid, sum(total_transitions_per_user) transitions from trans_detail10
# MAGIC group by uid

# COMMAND ----------

dummy = spark.sql("""
select * from silver_with_clicks_and_cost
""")
print(dummy.columns)
...
# MAGIC %sql
# MAGIC select uid, sum(impr) total_impr, sum(clicks) clicks, sum(txns) txns,  avg(impr) avg_impr_per_product, stddev_pop(impr) std_impr,
# MAGIC        sum(clicks) / sum(impr) as CTR, 
# MAGIC        CASE WHEN sum(clicks) > 0 THEN sum(txns) / sum(clicks) ELSE sum(txns) / sum(impr) END as CVR ,
# MAGIC        CASE WHEN sum(impr) > 0 THEN sum(txns) / sum(impr) ELSE 0 END as CVR2 
# MAGIC        
# MAGIC from
# MAGIC (
# MAGIC select
# MAGIC uid, product_stars, 
# MAGIC sum(visit_impr) as impr,
# MAGIC sum(visit_clicks) as clicks,
# MAGIC sum(visit_txns) as txns
# MAGIC from 
# MAGIC silver_with_clicks_and_cost
# MAGIC group by 
# MAGIC uid, product_stars
# MAGIC --order by uid, product_stars
# MAGIC )
# MAGIC group by uid, product_stars

# COMMAND ----------

['uid', 'time', 'channel', 'product_stars', 'visit_impr', 'visit_clicks', 'visit_txns', 'max_cpm', 'avg_cpi', 'bid_cpc', 'gpt', 'month', 'dayofweek', 'weekofyear', 'timestamp', 'sum_visit_impr', 'VISIT_RANK2']

# COMMAND ----------

# MAGIC %sql
# MAGIC SELECT *, p_end_state * conversions as p_conversion
# MAGIC FROM
# MAGIC (
# MAGIC select uid, end_state, 
# MAGIC         sum(end_state_probability) p_end_state,
# MAGIC         sum(conversion) conversions
# MAGIC 
# MAGIC         
# MAGIC FROM 
# MAGIC    transition_matrix_detail2 
# MAGIC group by uid, end_state
# MAGIC )
# MAGIC WHERE uid = '001253ff346144fb96725b5997e26611'
# MAGIC order by uid,end_state

# COMMAND ----------

# MAGIC %sql
# MAGIC SELECT end_state, sum(p_end_state) sum_p_end, sum( p_conversion) sum_p_conv,
# MAGIC        avg(p_end_state) avg_pend, avg(p_conversion) avg_p_conv,
# MAGIC        std(p_end_state) std_pend, std(p_conversion) std_p_conv
# MAGIC 
# MAGIC FROM
# MAGIC (
# MAGIC SELECT *, p_end_state * conversions as p_conversion
...
# MAGIC --order by uid,end_state
# MAGIC )
# MAGIC group by end_state

# COMMAND ----------

# MAGIC %sql
# MAGIC select start_state, end_state,
# MAGIC  sum(nstate_trainsitions) ntransitions,
# MAGIC  sum(conversion) nconversions,
# MAGIC  
# MAGIC  
# MAGIC  -avg(transition_probability) avg_transition_prob,
# MAGIC  -avg(start_state_probability) avg_start_prob
# MAGIC  
# MAGIC from 
# MAGIC transition_matrix_detail2
# MAGIC group by start_state, end_state

# COMMAND ----------

# MAGIC %sql
# MAGIC select uid, count(*) cnt
# MAGIC from transition_matrix_detail
# MAGIC group by uid
...
# MAGIC select * 
# MAGIC from 
# MAGIC   transition_matrix_detail
# MAGIC where end_state = 'Conversion'
