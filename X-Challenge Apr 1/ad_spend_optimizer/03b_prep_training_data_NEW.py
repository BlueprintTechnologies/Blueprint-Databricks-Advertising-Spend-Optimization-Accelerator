...
# MAGIC %sql
# MAGIC select * FROM bronze_with_clicks_and_cost WHERE uid = '00454ed87fed4c2e8844ad1aa0130b67'
...
# DBTITLE 1,Rank Customers by how active/visits
#%sql
df = spark.sql("""
    (SELECT 
...
# DBTITLE 1,Rank users according to how active/visits
# MAGIC %sql
# MAGIC SELECT VISITS_RANK, SUM(sum_visit_clicks) as visits
# MAGIC FROM 
# MAGIC (SELECT 
# MAGIC       NTILE(100) OVER ( ORDER BY sum_visit_impr DESC) AS VISITS_RANK,
# MAGIC       --NTILE(10) OVER (ORDER BY sum_visit_clicks DESC) AS CLICKS_RANK,
# MAGIC       *
# MAGIC FROM  contribution_per_customer) AS A
# MAGIC GROUP BY VISITS_RANK
...
# DBTITLE 1,Rank Users by how active/visits 2
...
print(bronze_clicks.columns)

# COMMAND ----------

# DBTITLE 1,summarize user behavior2
...
print(silver_clicks.columns)

# COMMAND ----------

display(silver_clicks.filter("clicks > impr"))
