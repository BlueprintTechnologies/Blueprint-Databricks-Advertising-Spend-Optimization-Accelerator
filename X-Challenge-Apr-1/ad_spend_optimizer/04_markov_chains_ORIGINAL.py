...
trans_mat = spark.table('transition_matrix')
display(trans_mat)

# COMMAND ----------

#trans_table_name = "transition_matrix"

#(trans_mat.write
#      .format("delta")
#      .mode("overwrite")
#      .saveAsTable(trans_table_name)
#)

# COMMAND ----------

# MAGIC %sql
# MAGIC select B.*, A.avg_trans_prob from transition_matrix B
# MAGIC LEFT JOIN
# MAGIC (
# MAGIC select start_state, count(*), avg(transition_probability) avg_trans_prob
# MAGIC from transition_matrix
# MAGIC group by start_state
# MAGIC ) A 
# MAGIC ON A.start_state = B.start_state

# COMMAND ----------

# MAGIC %sql select distinct end_state from transition_matrix

# COMMAND ----------

# MAGIC %sql
# MAGIC select start_state, sum(transition_probability), avg(transition_probability), std(transition_probability) 
# MAGIC from transition_matrix 
# MAGIC where end_state != 'Null' and start_state != 'Start'
# MAGIC group by start_state
...
display(spark.createDataFrame(transition_probability_pandas_df))
